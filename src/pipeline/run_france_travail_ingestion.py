"""Run France Travail raw ingestion.

Main deliverable for Task 005:
...
from src.common.config import DatabaseConfig, FranceTravailIngestionConfig
from src.common.database import get_connection
from src.common.logging import configure_logging, get_logger
from src.ingestion.france_travail import (
...
def _build_search_scopes(config: FranceTravailIngestionConfig) -> list[SearchScope]:
    """Build configured France Travail search scopes.

    Empty optional dimensions are treated as one open value so the default MVP
    run can work with only keywords configured.
    """

    locations = config.locations or [None]
    contract_types = config.contract_types or [None]
    remote_values = config.remote_values or [None]
...
def _build_requests(config: FranceTravailIngestionConfig) -> list[SearchRequest]:
    """Build all paginated France Travail requests for the run."""
...
def _sample_response_for_request(
    ...
    config: FranceTravailIngestionConfig,
) -> FranceTravailApiResponse:
    """Build a fake API response from a local sample JSON file.

    This exists only to make local raw loading testable without API credentials.
    It still uses the same loaders and raw table contract.
    """
...
def run_france_travail_ingestion(
    config: FranceTravailIngestionConfig | None = None,
    database_config: DatabaseConfig | None = None,
) -> IngestionSummary:
    """Run one France Travail raw ingestion batch."""

    ingestion_config = config or FranceTravailIngestionConfig.from_env()
    ...
    with get_connection(db_config) as connection:
        ...
        try:
            ...
            LOGGER.info("Built %s France Travail search request(s)", len(requests_to_run))

            client = None if ingestion_config.sample_mode else FranceTravailClient()
            ...
            for search_request in requests_to_run:
                ...
                try:
                    ...
                    if api_response.error_message or not _is_success_status(
                        ...
                        errors.append(error)
                        LOGGER.warning("France Travail request failed: %s", error)
                        continue
                    ...
                    # If the API call succeeded but the request row failed to persist,
                    # we cannot create raw rows safely because request_id is missing.
            ...
            LOGGER.exception("France Travail ingestion failed")
            try:
                connection.rollback()
                mark_batch_failed(
                    connection=connection,
...
def main() -> None:
    ...
    configure_logging()
    summary = run_france_travail_ingestion()
